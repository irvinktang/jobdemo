...
class Jobs extends React.Component {
  ...
  requestJob(event) {
    event.preventDefault();
    fetch(`/jobs/${this.state.reqId}`).then(res => res.json()).then(json => {
      var data = JSON.parse(json);
      this.setState({
        reqStatus: data.status,
        reqInfo: data.html || "Still processing...",
        showStatus: true
      });
    });
  }
      ...
      <div>
        ...
        <div className="col-sm-8">
          <table className="table table-bordered">
            ...
            <tbody>
              {this.state.jobData
                ? Object.keys(this.state.jobData).map(jobId => {
                    var data = this.state.jobData;
                    return (
                      <tr>
                        <td>{JSON.parse(data[jobId]).url}</td>
                        <td>{jobId}</td>
                      </tr>
                    );
                  })
